...
async def _get_ab_variant(company_id: str, industry: str) -> str:
    """A/Bテストのバリアントを自動割り当てする。

    outreach_performance テーブルに既存バリアントが2種以上あれば、
    送信数が最も少ないバリアントを均等配分のために優先選択する。
    テーブルが空または取得失敗の場合は "A" / "B" からランダム選択する。
    """
    try:
        ...
        result = (
            ...
            .select("email_variant, sent_count")
        ...
        if result.data:
            ...
            for row in result.data:
                v = row.get("email_variant", "default")
                variants[v] = variants.get(v, 0) + (row.get("sent_count", 0) or 0)

            if len(variants) >= 2:
                min_count = min(variants.values())
                candidates = [v for v, c in variants.items() if c == min_count]
                return random.choice(candidates)

    except Exception as e:
        logger.warning(f"A/B variant assignment failed (using default): {e}")

    # デフォルト: "A" と "B" からランダム選択
...
async def run_outreach_pipeline(
    ...
    # target_industry: デフォルトは製造業。他業種も industry パラメータで切り替え可能。
    ...
    try:
        ...
        if source == "google_sheets":
            ...
            # search_manufacturing_companies は map_to_company_data 済みの形式で返す
        ...
        if is_manufacturing and source in ("direct", "google_sheets"):
            ...
            for c in enriched_companies:
                ...
                # 業種テキストに製造業キーワードが含まれるか、明示的に製造業が指定されていれば通す
                if any(kw in industry_text for kw in MANUFACTURING_KEYWORDS) or \
                        "製造" in c.get("industry", ""):
                    filtered.append(c)
                else:
                    logger.debug(f"製造業フィルタで除外: {c.get('name', '')} (industry={c.get('industry', '')})")
        ...
        # business_overview が空の場合などに MANUFACTURING_PAIN_FALLBACK を仮設定する
        if is_manufacturing:
            for c in enriched_companies:
                c["_is_manufacturing_target"] = True
                # industry が未設定または汎用的な場合に "製造業" を補完
                if not c.get("industry") or c.get("industry") in ("", "その他"):
                    c["industry"] = "製造業"
                # 製造業向けペイン種別ヒントを context に追加（company_researcher が利用）
    ...
    try:
        async def _generate_lp_one(company: dict[str, Any]) -> dict[str, Any]:
            pain_summary = "; ".join(
                p.get("detail", "") for p in company.get("pain_points", [])[:2]
            )
    ...
    try:
        async def _compose_one(company: dict[str, Any]) -> dict[str, Any]:
            pain_summary = "; ".join(
                p.get("appeal_message", p.get("detail", ""))
                for p in company.get("pain_points", [])[:2]
            )

            # バリアント自動割り当て（company側に指定があれば優先）
            email_variant = company.get("email_variant") or await _get_ab_variant(
                company_id, company.get("industry", "")
            )
            ...
            if email_variant == "B":
                # バリアントB: 実績・事例強調型
                variant_instruction = (
                    "メール本文では、具体的な導入事例や数値実績を中心に訴求してください。"
                )
            elif email_variant == "C":
                # バリアントC: 課題解決型（ペイン重視）
                variant_instruction = (
                    "メール本文では、企業が抱える具体的な課題（ペイン）から入り、"
                    "解決策としてシャチョツーを提案してください。"
                )
            else:
                # バリアントA（デフォルト）: 標準型
                variant_instruction = ""
            ...
            try:
                ...
                if company.get("_is_manufacturing_target"):
                    fallback_subject = MANUFACTURING_SUBJECT_TEMPLATES.get(
                        email_variant, MANUFACTURING_SUBJECT_TEMPLATES["A"]
                    )
                else:
        ...
        for res in compose_results:
            ...
            composed.append({
                **res["company"],
                ...
                "email_variant": res["email_variant"],
            })
    ...
    try:
        form_connector = PlaywrightFormConnector(ConnectorConfig(
            connector_type="playwright_form",
            company_id=company_id,
            credentials={},
        ))
        ...
        leads_payload = [
            {
                ...
                "email_variant": next(
                    (c.get("email_variant", "A") for c in composed if c.get("name") == rec.company_name),
                    "A",
                ),
            }
            for rec in records
