...
# ── 料金定数（DB未設定時のフォールバック） ──────────────────────────────────
# 実際の料金はDB（pricing_modules / pricing_discounts）から取得する。
# DBに値がない場合のみ以下のデフォルト値を使用する。

PRICE_BRAIN = 30_000          # ブレイン月額（税抜）フォールバック
PRICE_BPO_CORE = 250_000      # BPOコア月額（税抜）フォールバック
PRICE_ADDITIONAL = 100_000    # 追加モジュール1個あたり月額（税抜）フォールバック
ANNUAL_DISCOUNT_RATE = Decimal("0.10")   # 年払い10%OFF フォールバック
TAX_RATE = Decimal("0.10")               # 消費税10%

# ── 公式料金プラン（CLAUDE.md 料金体系） ─────────────────────────────────────
# selected_modules に "common_bpo" または "industry_bpo" を渡すと
# 正式プランの単価が適用される。DB価格が取得できた場合はそちらを優先する。
...
async def _step1_calculate_quotation(
    ...
    try:
        from db.pricing import get_module_price, get_discount_rate

        line_items: list[dict[str, Any]] = []
        subtotal = Decimal("0")

        # DB から料金を取得（失敗時はフォールバック定数を使用）
        price_brain = await get_module_price(company_id, "brain")
        price_bpo_core = await get_module_price(company_id, "bpo_core")
        price_additional = await get_module_price(company_id, "additional")

        # 年払い割引率を DB から取得
        annual_discount_info = await get_discount_rate(company_id, "annual")
        annual_discount_rate = Decimal(
            str(annual_discount_info.get("rate_percent", "10.00"))
        ) / Decimal("100")
        ...
        # DB価格が取得できている場合は get_module_price の値を使い、
        # 未登録の場合は PRICING_PLANS のデフォルト単価にフォールバックする
        for plan_key, plan_info in PRICING_PLANS.items():
            if plan_key in selected_modules:
                # DB に plan_key のエントリがあればそちらを優先
                db_price = await get_module_price(company_id, plan_key)
                effective_price = db_price if db_price > 0 else plan_info["monthly_price"]
        ...
        if billing_cycle == "annual":
            ...
            if discount_amount > 0:
                line_items.append({
                    "name": f"年払い割引（{int(annual_discount_rate * 100)}%）",
                    "quantity": 1,
                    "unit": "式",
...
async def run_quotation_contract_pipeline(
    ...
    if approval_status != APPROVAL_APPROVED:
        ...
        # anomaly_detector: 見積単価・数量・合計の外れ値検知
        _phase_a_anomaly_items = [
            {"name": item["name"], "value": item["unit_price"]}
            for item in calc.get("line_items", [])
            if item.get("unit_price") and item["unit_price"] > 0
        ]
        if calc.get("subtotal"):
            _phase_a_anomaly_items.append({"name": "小計", "value": calc["subtotal"]})
        if calc.get("total"):
            _phase_a_anomaly_items.append({"name": "合計（税込）", "value": calc["total"]})

        _phase_a_anomaly_warnings: list[dict] = []
        if _phase_a_anomaly_items:
            try:
                _a_out = await run_anomaly_detector(MicroAgentInput(
                    company_id=company_id,
                    agent_name="anomaly_detector",
                    payload={
                        "items": _phase_a_anomaly_items,
                        "detect_modes": ["digit_error", "range"],
                    },
                ))
                if _a_out.success and _a_out.result.get("anomaly_count", 0) > 0:
                    _phase_a_anomaly_warnings = _a_out.result["anomalies"]
            except Exception as _ae:
                logger.warning(f"anomaly_detector (phase_a) 非致命的エラー（スキップ）: {_ae}")
        ...
        if _phase_a_anomaly_warnings:
            result.final_output["anomaly_warnings"] = _phase_a_anomaly_warnings
    ...
    # anomaly_detector: Phase B でも単価・合計の外れ値を再チェック
    _phase_b_anomaly_items = [
        {"name": item["name"], "value": item["unit_price"]}
        for item in calc.get("line_items", [])
        if item.get("unit_price") and item["unit_price"] > 0
    ]
    if calc.get("subtotal"):
        _phase_b_anomaly_items.append({"name": "小計", "value": calc["subtotal"]})
    if calc.get("total"):
        _phase_b_anomaly_items.append({"name": "合計（税込）", "value": calc["total"]})

    _phase_b_anomaly_warnings: list[dict] = []
    if _phase_b_anomaly_items:
        try:
            _b_out = await run_anomaly_detector(MicroAgentInput(
                company_id=company_id,
                agent_name="anomaly_detector",
                payload={
                    "items": _phase_b_anomaly_items,
                    "detect_modes": ["digit_error", "range"],
                },
            ))
            if _b_out.success and _b_out.result.get("anomaly_count", 0) > 0:
                _phase_b_anomaly_warnings = _b_out.result["anomalies"]
        except Exception as _be:
            logger.warning(f"anomaly_detector (phase_b) 非致命的エラー（スキップ）: {_be}")
    ...
    if _phase_b_anomaly_warnings:
        result.final_output["anomaly_warnings"] = _phase_b_anomaly_warnings
    result.total_cost_yen = sum(s.cost_yen for s in result.steps)
    result.total_duration_ms = int(time.time() * 1000) - pipeline_start
