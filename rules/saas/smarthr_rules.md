...
## UI/UX 設計ルール（従業員データ品質）

### 氏名の入力ルール

- 姓と名は分けて入力する（`last_name` + `first_name`）
- 全角カタカナ / ひらがな / 漢字を適切に使い分ける
- ふりがなフィールドがある場合は必ずセットで入力する

### メールアドレスの品質

- 個人メールではなく会社ドメインのメールアドレスを使用する
- 重複チェック: 登録前に `smarthr_list_crews` で同一メールが存在しないか確認
- フォーマット: `taro.yamada@example.com`（氏名ベースが一般的）

### 部署の設定

- `department_id` は必ず `smarthr_list_departments` で取得した実在の ID を使用
- 部署が不明な場合は空にせず、「未配属」等の適切な部署を指定する
- 組織変更で部署が変わった場合は更新漏れに注意

### 一括登録時の注意

- 大量登録（10件以上）は分割して実行する
- 各レコードのメールアドレスが一意であることを事前に確認する
- 登録順序: 部署マスタの確認 → 雇用形態の確認 → 従業員登録
