...
| **routers/connector.py** | 🔧 スケルトンのみ | 接続管理CRUD。kintone/freee API接続は未実装 |
| LangGraph統合（HitL） | ❌ 未実装 | Phase 2+ |
| Shadow Mode | ❌ 未実装 | Phase 2+ |
| ヒストリカルシミュレーション | ❌ 未実装 | Phase 2+ |
| workers/engineer/ | ❌ 未着手 | Phase 2+ |
...
- [ ] 統一コネクタレイヤー（API > iPaaS 自動選択）※ routers/connector.py スケルトンのみ実装済
- [ ] 共通SaaSコネクタ（kintone / freee / Slack / LINE WORKS / ジョブカン）
- [ ] 業種固有SaaSコネクタ（ANDPAD / Photoruction / zaico / Dentis）
- [ ] 承認チェックポイントUI（計画表示 / 修正 / 承認 / 却下）※ workers/bpo/engine/approval_workflow.py でバックエンド実装済
- [ ] ROIリアルタイム計測 + ダッシュボード
- [ ] 棄却学習（却下理由 → 次回の計画改善）
...
■ 製造業BPO ROI（50名・月額¥300K 想定）
  ...
  ROI: (¥970,000 - ¥250,000) / ¥250,000 = 288%/月（BPOコア¥250,000基準）
