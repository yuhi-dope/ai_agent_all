...
│  ・RT-DETR or YOLO-NAS で壁・柱・開口部・階段等を検出 │
│  ・セマンティックセグメンテーション（壁領域分割）      │
│  ・転移学習: BIM生成データ + パイロット企業図面         │
│  ・マルチモーダルLLM併用:                              │
│    CVが低確信度の部材 → Gemini Visionに判定委託       │
│                                                       │
│  ★ ライセンス判断:                                     │
│    YOLOv8 = AGPL-3.0（SaaS提供時にソースコード公開義務）│
│    RT-DETR / YOLO-NAS = Apache 2.0（公開義務なし）     │
│    → Apache 2.0モデルを採用。LICENSE表示のみで商用自由 │
│                                                       │
│  検出クラス（Phase 2）:                                │
│    壁(耐力壁/間仕切り), 柱, 梁, ドア, 窓,            │
│    階段, 通り芯, 寸法線, 建具記号                     │
...
│    配筋記号, 断面詳細, 仕口, 設備記号                 │
│                                                       │
│  技術: RT-DETR / YOLO-NAS (Apache 2.0), SegNeXt,      │
│        Gemini Vision                                   │
│  精度目標: 85-90%（mAP@0.5）                          │
└──────────────────┬──────────────────────────────────┘
                   ▼
...
| ソース | 入手方法 | 量 | コスト | 用途 |
|---|---|---|---|---|
| **CubiCasa5k** | GitHub公開 (github.com/CubiCasa/CubiCasa5k) | 5,000枚 + SVGアノテーション | 無料 | **⚠️ 実験・研究用のみ（CC BY-NC-SA 4.0 = 商用NG）。本番モデルには含めない** |
| **Roboflow Floor Plans** | Roboflow Universe | 500枚 + アノテーション | 無料 | 学習パイプライン検証用 |
...
対策: CubiCasaは実験用のみ（CC BY-NC-SA 4.0 = 商用利用NG）
  本番モデルの訓練データ:
  ① 国交省BIM → 2D図面自動生成（公開データ、商用OK）
  ② パイロット企業の実図面（許諾取得）
  ③ 確認UIの修正データ（自社蓄積）
  → CubiCasa5kは一切含めない → ライセンス問題なし
```
...
```
Stage 1（Phase 0）: BIM生成データ + 公開データでベースモデル構築
  データ: BIM→2D自動生成 300枚 + Roboflow公開データ
  学習: 壁・開口部・部屋領域の基本概念
  ※ CubiCasa5kは実験で精度検証に使うが、本番モデルには含めない（CC BY-NC-SA）
  精度: 日本図面に適用で60-70%
...
# Layer 3: 物体検出
# ★ ライセンス: Apache 2.0を選択（ソースコード公開義務なし）
#   YOLOv8 = AGPL-3.0（SaaS提供時にソースコード公開義務 → 不採用）
#   RT-DETR / YOLO-NAS = Apache 2.0（LICENSE表示のみで商用自由 → 採用）
torch==2.2+               # PyTorch
torchvision==0.17+        # 画像変換
# RT-DETR: pip install transformers  (Hugging Face経由)
# YOLO-NAS: pip install super-gradients  (Deci AI提供)
...
| リソース | 必要量 | 調達方法 | コスト |
|---|---|---|---|
| **積算士（業務委託・段階縮小）** | Phase1: フル稼働 → Phase3: 月4時間 → 卒業 | 業務委託 | Phase1: ¥30-50万/月 → Phase3: ¥2-5万/月 → ¥0 |
...
Phase 0（1ヶ月）: 前処理 + OCR + ベースモデル
  ...
  └─ Week 3-4: BIM生成データでRT-DETR/YOLO-NAS転移学習（並列）
  成果: 日本図面で壁・ドア・窓を70%+検出
...
| 積算士（業務委託・段階縮小） | ¥120-200万 | Phase1: ¥30-50万×3ヶ月 + Phase2: ¥15-25万×3ヶ月 + Phase3: ¥2-5万×4ヶ月。最終的に¥0 |
| 建設物価DB | ¥5万 | 書籍版年間 |
...
> **目標**: 積算士を業務委託で3-6ヶ月確保し、その間にAIが積算士のナレッジを吸収。
> 最終的にAIだけで回る体制に移行し、人件費ゼロを実現する。

#### Phase 1（Month 1-3）: 積算士に働いてもらいながらナレッジを記録

```
積算士の全ての判断をデータとして記録する:

① 図面の読み方（視覚パターン）:
   確認UIで積算士が修正 → 修正前後のペアが訓練データに
   例: AIが「柱」と判定 → 積算士が「PS（パイプスペース）」に修正
   → 次回から同パターンをPSと判定
   蓄積ペース: 月100-200件

② 数量の拾い方（計算手順）:
   積算士に「どこから何をどう計算したか」を構造化入力してもらう
   例: 壁面積 = 通り芯間距離6m × 階高3.5m - 開口部2箇所 = 13.44m²
   蓄積ペース: 月50-100件

③ 工種分類のルール（分類知識）:
   正規化辞書 + 分類ルールの自動蓄積
   既存のCATEGORY_RULES + term_normalizationを拡張
   蓄積ペース: 月30-60件

④ 単価の判断根拠（価格知識）:
   learn_from_result()を拡張して「理由」も記録
   例: 山間部→運搬距離長い→生コン単価1.23倍
   蓄積ペース: 月100-200件

⑤ 経験則・例外判断（暗黙知）:
   月1回のヒアリングセッション + 判断ログ
   例: 「曲面壁は型枠ロス1.2倍」「地下工事は地上の1.3-1.5倍」
   蓄積ペース: 月10-20ルール
```

#### Phase 2（Month 4-6）: AIが半分やる

```
LLM Vision + 蓄積データで①②のドラフトを自動生成
積算士は「ゼロから拾う」→「AIの結果を確認・修正する」に変化
作業時間: 1案件3時間 → 1時間
修正内容が全て次の学習データに
担当社数: 4-8社 → 8-15社にスケール
```

#### Phase 3（Month 7-10）: 積算士の役割を縮小

```
60-120案件分のナレッジ蓄積 + 図面解析AIの精度向上
AIが①②を85-90%自動化
積算士: 週1回の品質チェックのみ → 月4時間（¥2-5万）に縮小
あるいは完全にさよなら → スポット依頼のみ
```
...
| 種類 | 蓄積件数 | 活用方法 |
|---|---|---|
| 図面パターン | 300-600件 | 物体検出モデル再訓練 / LLMプロンプト改善 |
| 計算手順 | 150-300件 | ルールエンジンに組み込み |
| 分類ルール | 90-180件 | CATEGORY_RULES + term_normalization |
| 単価根拠 | 300-600件 | 単価推定の精度向上 |
| 経験則 | 30-60ルール | 補正係数の自動適用 |
| **合計** | **約1,000-1,700件** | **積算士の脳の80%をDB化** |

#### コスト推移と粗利率の変化

```
Month 1-3:  積算士¥30-50万/月（フル稼働）→ 粗利率 50-60%
Month 4-6:  積算士¥15-25万/月（半日稼働）→ 粗利率 65-75%
Month 7-10: 積算士¥2-5万/月（週1品質チェック）→ 粗利率 80-85%
Month 11-:  積算士¥0（スポット依頼のみ）→ 粗利率 85-90%
```

#### 技術的に吸収できない残り20%

```
・「この設計者の図面はいつも寸法が甘い」→ 個人の信頼度
・「この元請けとは過去にトラブルがあった」→ 人間関係
・「この現場は搬入路が狭い」→ 現地情報
・「今月は鉄筋が値上がりしそう」→ 市況の勘

→ これらは社長自身が知っている情報
→ 社長に最終確認してもらえば積算士なしでも補える
```

### 17.15 積算士なしルート（小規模工事特化・LLM Vision）

> 積算士を確保できない場合の代替戦略。マルチモーダルLLMに図面を直接投げる。
> 対象: 小規模民間工事（住宅リフォーム、小規模改修、外構、解体）

#### アプローチ

```
専用CVモデル（RT-DETR等）の訓練を待たず、
Gemini 2.5 Flash / Claude 4.5 のVision機能で図面を直接解析する。

Step 1: 図面画像をLLM Visionに投げる
  → 「この図面から部材・寸法・数量を抽出してください」
Step 2: 抽出結果を社長に確認してもらう（5-10分）
Step 3: 修正済み数量を estimator.py に渡す（③④⑤⑥は自動）
```

#### 精度（図面の種類別）

| 図面の種類 | LLM Vision精度 | 社長確認後の精度 |
|---|---|---|
| 小規模（住宅リフォーム・改修） | 70-80% | ±10-15% |
| 中規模（3階建て事務所） | 50-60% | ±15-20% |
| 大規模（マンション・公共施設） | 30-40% | 対象外（積算士ありルートを使用） |

#### 積算士ありルート vs LLM Visionルートの比較

| | 積算士ありルート（17.14） | LLM Visionルート（17.15） |
|---|---|---|
| 対象工事 | 全規模 | 小規模のみ |
| 最終精度 | ±5% | ±10-15%（社長確認後） |
| 人件費 | ¥30-50万/月（Phase 1）→ ¥0 | ¥0（初日から） |
| 開始まで | 積算士確保が必要 | 即日 |
| 開発期間 | 4-6週間 | 2ヶ月 |
...
```
案件規模で自動振り分け:
  小規模民間工事（500万円以下）→ LLM Visionルート（積算士不要）
  中大規模工事（500万円超）     → 積算士ありルート（Phase 1-3でAI移行）
→ 最初から両方提供し、幅広い案件に対応
```
