...
class SmartHRAdapter(SaaSMCPAdapter):
    ...
    async def connect(self, credentials: SaaSCredentials) -> None:
        ...
        await self._api_request(
            "GET", f"{self._instance_url}/api/v1/users/me",
        )
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning("SmartHR health_check 失敗", exc_info=True)
            return False
    ...
    async def execute_tool(
        ...
        base = f"{self._instance_url}/api/v1"

        if tool_name == "smarthr_list_crews":
            params: dict[str, Any] = {}
            ...
            return await self._api_request("GET", f"{base}/crews", params=params)

        if tool_name == "smarthr_get_crew":
            crew_id = arguments["crew_id"]
            return await self._api_request("GET", f"{base}/crews/{crew_id}")

        if tool_name == "smarthr_create_crew":
            body: dict[str, Any] = {
            ...
            if arguments.get("department_id"):
                body["department_id"] = arguments["department_id"]
            # 追加フィールドがあればマージ
            for k, v in arguments.items():
                if k not in ("last_name", "first_name", "email", "department_id"):
                    body[k] = v
            return await self._api_request("POST", f"{base}/crews", json=body)

        if tool_name == "smarthr_update_crew":
            crew_id = arguments["crew_id"]
            return await self._api_request(
                "PATCH", f"{base}/crews/{crew_id}",
                json=arguments["fields"],
            )

        if tool_name == "smarthr_list_departments":
            return await self._api_request("GET", f"{base}/departments")

        if tool_name == "smarthr_list_employment_types":
            return await self._api_request("GET", f"{base}/employment_types")

        if tool_name == "smarthr_get_payroll_statement":
            crew_id = arguments["crew_id"]
            params = {
                "year": arguments["year"],
            ...
            return await self._api_request(
                "GET", f"{base}/crews/{crew_id}/payroll_statements",
                params=params,
            )

        raise ValueError(f"SmartHR: 不明なツール '{tool_name}'")
