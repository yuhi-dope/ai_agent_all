...
| 最適化・テスト | Review 内で生成コードを `work_dir` に書き出し、Lint/Build/Unit/E2E を実行。合格時のみ status が `published` となり PR 作成。 |
...
| review 合格 | `status == "published"`。Secret Scan / Lint・Build / Unit / E2E のすべて通過かつ PR 変更量 200 行以内。 |
...
- **PR 変更量**: 生成コードの総行数が 200 行以内であること（`MAX_LINES_PER_PR`）。

これらは [develop_agent/nodes/review_guardrails.py](develop_agent/nodes/review_guardrails.py) で順に実行され、すべて通過した場合にのみ status が `published` となり、GitHub Publisher で PR が作成されます。E2E は [develop_agent/utils/guardrails.py](develop_agent/utils/guardrails.py) の `run_e2e_test` で Playwright を実行し、未導入時は `passed=True` でスキップされます。
...
1. **自社の Next.js リポジトリを GitHub に push する**  
   develop_agent が PR を作成し、マージされたブランチ（例: `main`）に成果が含まれている状態にする。
...
4. **デプロイする**  
   ...
   - 任意: develop_agent が PR を出す先のブランチと、Vercel の Production Branch を一致させておくと、マージされた時点で本番に反映される。
